...
- [ ] **footprint縮小（16x16→12x12相当）に伴う`entity/`テクスチャの再作成が必要（未着手・ユーザーによるアセット作業待ち）**: `MultiItemFrameRenderer`の`FOOTPRINT_HALF`導入により、フレームの実際の描画面はブロック全体（16px相当）ではなく中央12x12（＝1ブロック=16pxの慣習に対して12/16倍）に縮小されたが、各テクスチャのUV座標は引き続き0〜1で描画面全体に引き伸ばして参照する仕組みのまま（コード側は変更不要・縮小された描画面へ自動的に収まる）。ただし、これまで「16x16=1ブロック全面」という前提でドット絵を用意した`entity/`テクスチャ（1px=1ゲーム内ピクセルの密度を想定）は、実際の描画面が12x12相当に縮んだことで想定していたドット密度とズレる（16x16の絵をそのまま12x12相当の面に収めると等倍表示にならず、若干のスケーリングがかかる）。混乱を避けるため、以下のテクスチャは実サイズを12x12（縦横比が変わらないもの）に作り直すことを推奨:
  - `common/src/main/resources/assets/multiitemframe/textures/entity/background.png`（現状16x16 → **12x12**。スロットごとの背景。1スロット全体を1ブロックとみなした密度で作られているため、footprint基準に合わせて縮小）
  - `common/src/main/resources/assets/multiitemframe/textures/entity/highlight_frame.png`（現状16x16 → **12x12**。外周の枠テクスチャ。中心の透明領域と外周の単色ボーダーの比率を保ったまま縮小）
  - `common/src/main/resources/assets/multiitemframe/textures/entity/highlight_fill.png`（現状16x16 → **12x12**。塗りつぶし用の単色/半透明テクスチャ、単色なら実質サイズ非依存だが統一のため揃える）
  - `common/src/main/resources/assets/multiitemframe/textures/entity/frame.png`（現状32x32＝16x16の2倍解像度想定 → **24x24**（12x12の2倍解像度を維持する場合）または**12x12**（等倍に統一する場合）。フレーム前面/背面の板テクスチャ、描画面全体に貼られるため上記と同じ理由で縮小対象）
  - `common/src/main/resources/assets/multiitemframe/textures/entity/frame_side.png`（現状16x16 → 長さ方向のみ**12px相当に短縮**、厚み方向（1px相当のライン）は現状維持。フレームの側面帯は描画面の外周（今回12/16に縮んだ辺）に沿って貼られるため、長さ方向だけ影響を受ける）
  - **対象外（変更不要）**: `entity/frame_glow.png`・`entity/frame_glow_side.png`は前回の変更でレンダラーから未参照になったため、今回のfootprint縮小の影響を受けない（削除するかは別途判断）。`item/frame_*.png`・`item/glow_frame_*.png`（インベントリアイコン、16x16）はインワールドの描画面とは無関係（別モデル・別UVのため）で対象外。

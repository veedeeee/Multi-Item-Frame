...
/**
 * Resolves the atlas sprite + tint to draw for a non-{@code ITEM} slot content (Fluid/Gas/
 * Infusion/Pigment/Slurry/Energy - see {@link DisplayContentKind}), shared by both the in-world
 * renderer ({@code MultiItemFrameRenderer}) and the settings GUI ({@code MultiItemFrameScreen}).
 *
...
final class ContentIconResolver {
    ...
    private static final ResourceLocation ENERGY_ICON = new ResourceLocation("mekanism", "liquid/energy");

    ...
    public void render(MultiItemFrameEntity entity, float yaw, float partialTicks, PoseStack poseStack,
        ...
        for (int slot = 0; slot < size.slotCount(); slot++) {
            ...
            if (contentKind == DisplayContentKind.ITEM) {
                ...
                // Fluid/Gas/Infusion/Pigment/Slurry/Energy: drawn as a flat tinted quad sampling
                // the same atlas sprite the game already uses for that content (see
                // ContentIconResolver), instead of going through ItemRenderer.

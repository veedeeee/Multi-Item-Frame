...
public class MultiItemFrameMenu extends AbstractContainerMenu {
    ...
    /** One slot's worth of direct-item ids. */
     ...
     *  to drag for either - items already have their own range above, and generic FE energy has
     *  no per-type identity to encode) - kept for simple, stable ordinal-based indexing. */
    ...
    /**
     * Sets a slot's displayed Fluid/Chemical content directly by Forge's per-session integer
     * registry id (used by the JEI raw-ingredient-drag ghost handler: JEI's ghost-ingredient
     * click mechanism can only transmit a plain {@code int}, not a {@code FluidStack}/chemical
     * stack, so the dragged ingredient's type is resolved to a registry id client-side and
     * decoded back here - see {@code MultiItemFrameScreen#sendDirectContent}).
     */
    private void setDisplayContentDirect(int slot, DisplayContentKind kind, int registryId) {
        ...
        String contentId = switch (kind) {
            ...
            case GAS, INFUSION, PIGMENT, SLURRY -> MekanismChemicalCompat.resolveContentId(kind, registryId);
            default -> null;
        };
        if (contentId != null) {
